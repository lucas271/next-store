...
export const paymentValidation = z.object({
  cardHolder: z.string().nonempty("Preencha o campo").toUpperCase(),
  cardNumber: z.string().nonempty("Preencha o campo")
  .refine((value) => {
  ...
  CVV: z.string().nonempty("Preencha o campo").min(3, 'Mínimo 3 caracteres').max(4, 'Máximo 4 caracteres'),
  paymentType: z.string().nonempty('Selecione um metodo').toUpperCase().refine((input) => {
