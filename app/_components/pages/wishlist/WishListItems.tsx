...
const WishListItems = () => {
	...
	return <>
		<section className={`m-auto px-3 w-full my-3 grid grid-cols-1 flex-grow sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 `}>
			{wishList.errors.length > 0 && <div className="w-full col-span-full rounded-lg p-4">
				{wishList.errors.map((errMsg, index) => <span key={index} >
					<ErrorMessage message={errMsg}/>
				</span>
				)}
				<StyledButton text="Atualizar" onClick={() => dispatch(getWishListItems())} className={'p-3'}/>
			</div>}
